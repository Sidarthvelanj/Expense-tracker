...
function Navbar() {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="sm:hidden fixed top-0 left-0 w-full bg-white dark:bg-[#1f1f2e] shadow-md z-50 flex items-center justify-between px-4 py-3">
        <h1 className="text-lg font-bold text-gray-800 dark:text-white">💸</h1>
        <button onClick={() => setMobileOpen(!mobileOpen)} className="text-gray-700 dark:text-gray-300">
          {mobileOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      {/* Mobile Sidebar */}
      <div className={`sm:hidden fixed top-14 left-0 w-full bg-white dark:bg-[#1f1f2e] shadow-lg z-40 transition-all duration-300 ${mobileOpen ? 'block' : 'hidden'}`}>
        <nav className="flex flex-col px-4 py-2 space-y-2">
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center gap-3 px-4 py-2 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
          >
      ...
      {/* Desktop Sidebar */}
      <div className={`hidden sm:flex fixed top-0 left-0 h-screen bg-white dark:bg-[#1f1f2e] shadow-lg transition-all duration-300 z-50 flex-col justify-between ${navbarCollapsed ? 'w-20' : 'w-64'}`}>
        <div>
